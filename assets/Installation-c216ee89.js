import{o as s,c as n,a as e}from"./index-1d9d7dc5.js";const t={class:"md-content__inner md-typeset"},c={__name:"Installation",setup(i){return(p,a)=>(s(),n("article",t,a[0]||(a[0]=[e(`<h1 id="installation"> 설치 <a class="headerlink" href="#installation" title="영구 링크"> ¶ </a></h1><p>이 페이지는 봇을 실행하기 위해 환경을 준비하는 방법을 설명합니다.</p><p>Freqtrade 문서는 Freqtrade를 설치하는 다양한 방법을 설명합니다.</p><ul><li><a href="docker_quickstart.html"> Docker 이미지 </a> (별도 페이지) </li><li><a href="#script-installation"> 스크립트 설치 </a></li><li><a href="#manual-installation"> 수동 설치 </a></li><li><a href="#installation-with-conda"> Conda를 사용한 설치 </a></li></ul><p> Freqtrade의 작동 방식을 평가하는 동안 빠르게 시작하려면 미리 빌드된 <a href="docker_quickstart.html"> Docker 이미지 </a> 를 사용하는 것을 고려하십시오. </p><hr><h2 id="information"> 정보 <a class="headerlink" href="#information" title="영구 링크"> ¶ </a></h2><p> Windows 설치의 경우 <a href="windows_installation.html"> Windows 설치 가이드 </a> 를 사용하십시오. </p><p> Freqtrade를 설치하고 실행하는 가장 쉬운 방법은 Github 저장소를 클론한 다음 플랫폼에 맞는 <code> ./setup.sh </code> 스크립트를 실행하는 것입니다. </p><div class="admonition note"><p class="admonition-title">버전 고려 사항</p><p> 저장소를 클론할 때 기본 작업 브랜치는 <code> develop </code> 입니다. 이 브랜치는 최신 기능을 포함하며, 자동화된 테스트 덕분에 상대적으로 안정적이라고 간주될 수 있습니다. <code> stable </code> 브랜치는 마지막 릴리스의 코드를 포함하며, 일반적으로 한 달에 한 번, 약 일주일 된 <code> develop </code> 브랜치의 스냅샷을 기반으로 릴리스됩니다. 따라서 더 안정적일 가능성이 있습니다. </p></div><div class="admonition note"><p class="admonition-title">참고</p><p> Python 3.10 이상과 해당 <code> pip </code> 이 사용 가능해야 합니다. 설치 스크립트는 그렇지 않은 경우 경고를 표시하고 중지됩니다. <code> git </code> 도 Freqtrade 저장소를 클론하는 데 필요합니다. <br> 또한 설치가 성공적으로 완료되려면 Python 헤더( <code> python&lt;버전&gt;-dev </code> / <code> python&lt;버전&gt;-devel </code> )가 사용 가능해야 합니다. </p></div><div class="admonition warning"><p class="admonition-title">정확한 시계</p><p>봇을 실행하는 시스템의 시계는 정확해야 하며, 거래소와의 통신 문제를 방지하기 위해 NTP 서버와 자주 동기화되어야 합니다.</p></div><hr><h2 id="requirements"> 요구 사항 <a class="headerlink" href="#requirements" title="영구 링크"> ¶ </a></h2><p> 이 요구 사항은 <a href="#script-installation"> 스크립트 설치 </a> 와 <a href="#manual-installation"> 수동 설치 </a> 에 모두 적용됩니다. </p><div class="admonition note"><p class="admonition-title">ARM64 시스템</p><p> ARM64 시스템(MacOS M1 또는 Oracle VM과 같은)을 실행 중인 경우 <a href="docker_quickstart.html"> Docker </a> 를 사용하여 Freqtrade를 실행하십시오. 네이티브 설치는 수동 작업이 필요하지만 현재는 지원되지 않습니다. </p></div><h3 id="install-guide"> 설치 가이드 <a class="headerlink" href="#install-guide" title="영구 링크"> ¶ </a></h3><ul><li><a href="http://docs.python-guide.org/en/latest/starting/installation/"> Python &gt;= 3.10 </a></li><li><a href="https://pip.pypa.io/en/stable/installing/"> pip </a></li><li><a href="https://git-scm.com/book/en/v2/Getting-Started-Installing-Git"> git </a></li><li><a href="https://virtualenv.pypa.io/en/stable/installation.html"> virtualenv </a> (권장) </li><li><a href="https://ta-lib.github.io/ta-lib-python/"> TA-Lib </a> (설치 지침 <a href="#install-ta-lib"> 아래 </a> 참조) </li></ul><h3 id="install-code"> 설치 코드 <a class="headerlink" href="#install-code" title="영구 링크"> ¶ </a></h3><p>우리는 Ubuntu, MacOS 및 Windows에 대한 설치 지침을 포함/수집했습니다. 이는 가이드라인이며 다른 배포판에서는 성공 여부가 다를 수 있습니다. OS별 단계가 먼저 나열되며, 아래의 공통 섹션은 모든 시스템에 필요합니다.</p><div class="admonition note"><p class="admonition-title">참고</p><p>Python 3.10 이상과 해당 pip이 사용 가능해야 합니다.</p></div><div class="tabbed-set tabbed-alternate" data-tabs="1:3"><input checked="checked" id="__tabbed_1_1" name="__tabbed_1" type="radio"><input id="__tabbed_1_2" name="__tabbed_1" type="radio"><input id="__tabbed_1_3" name="__tabbed_1" type="radio"><div class="tabbed-labels"><label for="__tabbed_1_1"> Debian/Ubuntu </label><label for="__tabbed_1_2"> MacOS </label><label for="__tabbed_1_3"> RaspberryPi/Raspbian </label></div><div class="tabbed-content"><div class="tabbed-block"><h4 id="install-necessary-dependencies"> 필요한 종속성 설치 <a class="headerlink" href="#install-necessary-dependencies" title="영구 링크"> ¶ </a></h4><div class="highlight"><pre><code><span class="c1"># 저장소 업데이트</span>
...
</code></pre></div><div class="admonition note"><p class="admonition-title">설치 시간</p><p> 인터넷 속도와 Raspberry Pi 버전에 따라 설치가 완료되는 데 여러 시간이 걸릴 수 있습니다. 따라서 Raspberry Pi용으로 미리 빌드된 Docker 이미지를 사용하는 것이 좋습니다. 자세한 내용은 <a href="docker_quickstart.html"> Docker 빠른 시작 문서 </a> 를 참조하십시오. </p></div><div class="admonition note"><p class="admonition-title">참고</p><p> 위의 내용은 hyperopt 종속성을 설치하지 않습니다. 이를 설치하려면 <code> python3 -m pip install -e .[hyperopt] </code> 를 사용하십시오. Raspberry Pi에서 hyperopt를 실행하는 것은 매우 리소스를 많이 사용하는 작업이므로 강력한 컴퓨터에서 수행하는 것이 좋습니다. </p></div></div></div></div><hr><h2 id="freqtrade-repository"> Freqtrade 저장소 <a class="headerlink" href="#freqtrade-repository" title="영구 링크"> ¶ </a></h2><p> Freqtrade는 오픈 소스 암호화폐 거래 봇으로, 코드는 <code> github.com </code> 에 호스팅됩니다. </p><div class="highlight"><pre><code><span class="c1"># Freqtrade 저장소의 \`develop\` 브랜치 다운로드</span>
git<span class="w"> </span>clone<span class="w"> </span>https://github.com/freqtrade/freqtrade.git
...
</code></pre></div><p><a href="#you-are-ready"> 이제 준비가 완료되었습니다. </a> 봇을 실행할 수 있습니다. </p><h3 id="optional-post-installation-tasks"> (선택 사항) 설치 후 작업 <a class="headerlink" href="#optional-post-installation-tasks" title="영구 링크"> ¶ </a></h3><div class="admonition note"><p class="admonition-title">참고</p><p> 서버에서 봇을 실행하는 경우 <a href="docker_quickstart.html"> Docker </a> 또는 <code> screen </code> 과 같은 터미널 멀티플렉서를 사용하거나 <a href="https://en.wikipedia.org/wiki/Tmux"><code> tmux </code></a> 를 사용하여 로그아웃 시 봇이 중지되지 않도록 고려하십시오. </p></div><p> Linux에서 <code> systemd </code> 소프트웨어 스위트를 사용하는 경우, 선택적으로 봇을 <code> systemd 서비스 </code> 로 실행하거나 로그 메시지를 <code> syslog </code> / <code> rsyslog </code> 또는 <code> journald </code> 데몬으로 전송하도록 설정할 수 있습니다. 자세한 내용은 <a href="advanced-setup.html"> 고급 로깅 </a> 을 참조하십시오. </p><h3 id="setup-python-virtual-environment-virtualenv"> Setup Python virtual environment (virtualenv) <a class="headerlink" href="#setup-python-virtual-environment-virtualenv" title="Permanent link"> ¶ </a></h3><p> You will run freqtrade in separated <code> virtual environment </code></p><div class="highlight"><pre><code><span class="c1"># create virtualenv in directory /freqtrade/.venv</span>
python3<span class="w"> </span>-m<span class="w"> </span>venv<span class="w"> </span>.venv
...
</code></pre></div><p>이 파일이 존재하지 않는 경우, 다른 MacOS 버전을 사용 중일 가능성이 있으므로 인터넷에서 특정 해결 방법을 찾아보십시오.</p>`,124)])))}};export{c as default};
