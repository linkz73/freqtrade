...
</code></pre></div><div class="admonition danger"><p class="admonition-title"><code> adjust_*_price() </code> 와의 비호환성 </p><p><code> adjust_order_price() </code> 와 <code> adjust_entry_price() </code> / <code> adjust_exit_price() </code> 를 모두 구현한 경우 <code> adjust_order_price() </code> 만 사용됩니다. 진입/종료 가격을 조정해야 하는 경우 논리를 <code> adjust_order_price() </code> 에 구현하거나 분할된 <code> adjust_entry_price() </code> / <code> adjust_exit_price() </code> 콜백을 사용할 수 있지만 둘 다 사용할 수는 없습니다. 이를 혼합하는 것은 지원되지 않으며 봇 시작 시 오류가 발생합니다. </p></div><h3 id="adjust-entry-price"> 진입 가격 조정 <a class="headerlink" href="#adjust-entry-price" title="영구 링크"> ¶ </a></h3><p><code> adjust_entry_price() </code> 콜백은 전략 개발자가 진입 지정가 주문을 새로 고치거나 교체하는 데 사용할 수 있습니다. 이는 <code> adjust_order_price() </code> 의 하위 집합이며 진입 주문에 대해서만 호출됩니다. 나머지 동작은 <code> adjust_order_price() </code> 와 동일합니다. </p><p> 거래 오픈 날짜( <code> trade.open_date_utc </code> )는 첫 번째 주문이 실행된 시점에 유지됩니다. 이를 인식하고 다른 콜백에서 논리를 조정하여 첫 번째 채워진 주문의 날짜를 사용할 수 있도록 해야 합니다. </p><h3 id="adjust-exit-price"> 종료 가격 조정 <a class="headerlink" href="#adjust-exit-price" title="영구 링크"> ¶ </a></h3><p><code> adjust_exit_price() </code> 콜백은 전략 개발자가 종료 지정가 주문을 새로 고치거나 교체하는 데 사용할 수 있습니다. 이는 <code> adjust_order_price() </code> 의 하위 집합이며 종료 주문에 대해서만 호출됩니다. 나머지 동작은 <code> adjust_order_price() </code> 와 동일합니다. </p><h2 id="leverage-callback"> 레버리지 콜백 <a class="headerlink" href="#leverage-callback" title="영구 링크"> ¶ </a></h2><p>레버리지를 허용하는 시장에서 거래할 때 이 메서드는 원하는 레버리지를 반환해야 합니다(기본값은 1 -&gt; 레버리지 없음).</p><p>500USDT의 자본을 가정할 때, 레버리지=3인 거래는 500 x 3 = 1500 USDT의 포지션을 초래합니다.</p><p><code> max_leverage </code> 를 초과하는 값은 <code> max_leverage </code> 로 조정됩니다. 레버리지를 지원하지 않는 시장/거래소의 경우 이 메서드는 무시됩니다. </p><div class="highlight"><pre><span></span><code><span class="c1"># Default imports</span>
