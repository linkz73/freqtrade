import{o as s,c as e,a as n}from"./index-1d9d7dc5.js";const o={class:"md-content__inner md-typeset"},p={__name:"DataDownload",setup(t){return(d,a)=>(s(),e("article",o,a[0]||(a[0]=[n(`<h1 id="data-downloading"> 데이터 다운로드 <a class="headerlink" href="#data-downloading" title="영구 링크"> ¶ </a></h1><h2 id="getting-data-for-backtesting-and-hyperopt"> 백테스팅 및 하이퍼옵트용 데이터 가져오기 <a class="headerlink" href="#getting-data-for-backtesting-and-hyperopt" title="영구 링크"> ¶ </a></h2><p> 백테스팅 및 하이퍼옵트에 필요한 데이터(캔들 / OHLCV)를 다운로드하려면 <code> freqtrade download-data </code> 명령어를 사용하세요. </p><p> 추가 매개변수를 지정하지 않으면, freqtrade는 기본적으로 <code> &quot;1m&quot; </code> 및 <code> &quot;5m&quot; </code> 타임프레임의 최근 30일 데이터를 다운로드합니다. 거래소 및 거래쌍은 <code> config.json </code> 파일에서 가져옵니다(만약 <code> -c/--config </code> 옵션으로 지정된 경우). 설정 파일이 제공되지 않으면 <code> --exchange </code> 옵션이 필수입니다. </p><p> 상대적 시간 범위( <code> --days 20 </code> ) 또는 절대적 시작점( <code> --timerange 20200101- </code> )을 사용할 수 있습니다. 증분 다운로드의 경우 상대적 접근 방식을 사용하는 것이 좋습니다. </p><div class="admonition tip"><p class="admonition-title">팁: 기존 데이터 업데이트</p><p> 이미 데이터 디렉토리에 백테스팅 데이터가 있는 경우, freqtrade는 자동으로 누락된 시간 범위를 계산하여 최신 데이터까지 다운로드합니다. 이 경우 <code> --days </code> 또는 <code> --timerange </code> 매개변수가 필요하지 않습니다. freqtrade는 기존 데이터를 유지하고 누락된 데이터만 다운로드합니다. <br> 새로운 거래쌍을 추가한 후 기존 데이터를 업데이트하려면 <code> --new-pairs-days xx </code> 매개변수를 사용하세요. 지정된 일수만큼 새로운 거래쌍의 데이터를 다운로드하며, 기존 거래쌍은 누락된 데이터만 업데이트됩니다. </p></div><h3 id="usage"> 사용법 <a class="headerlink" href="#usage" title="영구 링크"> ¶ </a></h3><div class="highlight"><pre><code>사용법: freqtrade download-data [-h] [-v] [--no-color] [--logfile FILE] [-V]
...
</code></pre></div><div class="admonition tip"><p class="admonition-title">특정 화폐쌍의 모든 데이터 다운로드</p><p> 종종 특정 화폐쌍의 모든 데이터를 다운로드하고 싶을 때가 있습니다. 이 경우 다음과 같은 간단한 방법을 사용할 수 있습니다: <code> freqtrade download-data --exchange binance --pairs &quot;.*/USDT&quot; &lt;...&gt; </code> . 제공된 &quot;pairs&quot; 문자열은 거래소의 모든 활성 쌍을 포함하도록 확장됩니다. 비활성(상장 폐지된) 쌍의 데이터도 다운로드하려면 <code> --include-inactive-pairs </code> 를 명령어에 추가하세요. </p></div><div class="admonition note"><p class="admonition-title">시작 기간</p><p><code> download-data </code> 는 전략과 무관한 명령어입니다. 한 번에 많은 데이터를 다운로드한 후, 점진적으로 저장된 데이터의 양을 늘리는 것이 목표입니다. </p><p> 따라서, <code> download-data </code> 는 전략에서 정의된 &quot;시작 기간&quot;을 신경 쓰지 않습니다. 백테스트가 특정 시점에서 시작되도록 추가 일수를 다운로드하는 것은 사용자에게 달려 있습니다(시작 기간을 준수하면서). </p></div><h3 id="start-download"> 다운로드 시작 <a class="headerlink" href="#start-download" title="영구 링크"> ¶ </a></h3><p> 아주 간단한 명령어(사용 가능한 <code> config.json </code> 파일이 있다고 가정)를 다음과 같이 사용할 수 있습니다. </p><div class="highlight"><pre><code>freqtrade<span class="w"> </span>download-data<span class="w"> </span>--exchange<span class="w"> </span>binance
