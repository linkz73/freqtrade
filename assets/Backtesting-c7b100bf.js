import{o as a,c as s,a as t}from"./index-1d9d7dc5.js";const n={class:"md-content__inner md-typeset"},o={__name:"Backtesting",setup(d){return(c,e)=>(a(),s("article",n,e[0]||(e[0]=[t(`<h1 id="backtesting"> 백테스팅 <a class="headerlink" href="#backtesting" title="영구 링크"> ¶ </a></h1><p>이 페이지는 백테스팅을 사용하여 전략 성능을 검증하는 방법을 설명합니다.</p><p> 백테스팅에는 과거 데이터가 필요합니다. 관심 있는 거래쌍과 거래소의 데이터를 얻는 방법은 문서의 <a href="data-download.html"> 데이터 다운로드 </a> 섹션을 참조하세요. </p><h2 id="backtesting-command-reference"> 백테스팅 명령어 참조 <a class="headerlink" href="#backtesting-command-reference" title="영구 링크"> ¶ </a></h2><div class="highlight"><pre><code>사용법: freqtrade backtesting [-h] [-v] [--no-color] [--logfile FILE] [-V]
...
</code></pre></div><h2 id="test-your-strategy-with-backtesting"> 백테스팅으로 전략 테스트하기 <a class="headerlink" href="#test-your-strategy-with-backtesting" title="영구 링크"> ¶ </a></h2><p>이제 좋은 진입 및 종료 전략과 일부 과거 데이터를 보유하고 있다면, 이를 실제 데이터에 대해 테스트하고 싶을 것입니다. 이를 백테스팅이라고 합니다.</p><p> 백테스팅은 기본적으로 설정 파일에 있는 암호화폐(거래쌍)를 사용하며, <code> user_data/data/&lt;exchange&gt; </code>에서 과거 캔들(OHLCV) 데이터를 로드합니다. 해당 거래소/거래쌍/시간 프레임 조합에 대한 데이터가 없으면, 백테스팅은 먼저 <code> freqtrade download-data </code> 명령어를 사용하여 데이터를 다운로드하도록 요청합니다. 다운로드에 대한 자세한 내용은 문서의 <a href="data-download.html"> 데이터 다운로드 </a> 섹션을 참조하세요. </p><p>백테스팅 결과는 봇이 손실보다 이익을 낼 가능성이 더 높은지 확인합니다.</p><p>모든 수익 계산에는 수수료가 포함되며, freqtrade는 계산에 거래소의 기본 수수료를 사용합니다.</p><div class="admonition warning"><p class="admonition-title">동적 거래쌍 목록을 사용한 백테스팅</p><p> 동적 거래쌍 목록을 사용하는 것은 가능하지만(모든 핸들러가 백테스트 모드에서 사용 가능한 것은 아님), 현재 시장 상황에 의존합니다. 이는 거래쌍 목록의 과거 상태를 반영하지 않습니다. 또한, StaticPairlist 이외의 거래쌍 목록을 사용할 때 백테스팅 결과의 재현성을 보장할 수 없습니다. 자세한 내용은 <a href="plugins.html"> 거래쌍 목록 문서 </a>를 참조하세요. </p><p> 재현 가능한 결과를 얻으려면 <a href="utils.html"><code> test-pairlist </code></a> 명령어를 사용하여 거래쌍 목록을 생성하고 이를 정적 거래쌍 목록으로 사용하세요. </p></div><div class="admonition note"><p class="admonition-title">참고</p><p> 기본적으로 Freqtrade는 백테스팅 결과를 <code> user_data/backtest_results </code>에 내보냅니다. 내보낸 거래는 <a href="#further-backtest-result-analysis"> 추가 분석 </a>에 사용하거나 <a href="plotting.html"> 플로팅 하위 명령어 </a> ( <code> freqtrade plot-dataframe </code>)에서 사용할 수 있습니다. </p></div><h3 id="starting-balance"> 시작 잔액 <a class="headerlink" href="#starting-balance" title="영구 링크"> ¶ </a></h3><p> 백테스팅에는 시작 잔액이 필요하며, 이는 <code> --dry-run-wallet &lt;balance&gt; </code> 또는 <code> --starting-balance &lt;balance&gt; </code> 명령어 인수로 제공하거나 <code> dry_run_wallet </code> 구성 설정을 통해 제공할 수 있습니다. 이 금액은 <code> stake_amount </code>보다 높아야 하며, 그렇지 않으면 봇이 거래를 시뮬레이션할 수 없습니다. </p><h3 id="dynamic-stake-amount"> 동적 스테이크 금액 <a class="headerlink" href="#dynamic-stake-amount" title="영구 링크"> ¶ </a></h3><p> 백테스팅은 <a href="configuration.html"> 동적 스테이크 금액 </a>을 <code> stake_amount </code>를 <code> &quot;unlimited&quot; </code>로 설정하여 지원합니다. 이는 시작 잔액을 <code> max_open_trades </code> 조각으로 나눕니다. 초기 거래의 수익은 이후 더 높은 스테이크 금액으로 이어지며, 백테스팅 기간 동안 수익이 복리로 증가합니다. </p><h3 id="example-backtesting-commands"> 백테스팅 명령어 예제 <a class="headerlink" href="#example-backtesting-commands" title="영구 링크"> ¶ </a></h3><p>5분 캔들(OHLCV) 데이터로 (기본값)</p><div class="highlight"><pre><code>freqtrade<span class="w"> </span>backtesting<span class="w"> </span>--strategy<span class="w"> </span>AwesomeStrategy
...
</code></pre></div><h2 id="next-step"> 다음 단계 <a class="headerlink" href="#next-step" title="영구 링크"> ¶ </a></h2><p> 훌륭합니다. 전략이 수익성이 있습니다. 봇이 전략에 사용할 최적의 매개변수를 제공할 수 있다면 어떨까요? 다음 단계는 <a href="hyperopt.html"> 하이퍼옵트로 최적의 매개변수를 찾는 방법 </a>을 배우는 것입니다. </p>`,124)])))}};export{o as default};
