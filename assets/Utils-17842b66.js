import{o as i,c as a,a as n,b as s,u as r}from"./index-f254d819.js";const t="/assets/show-config-output-f69ae73f.png";const o={class:"md-content__inner md-typeset"},d=["src"],h={__name:"Utils",setup(l){return(c,e)=>(i(),a("article",o,[e[0]||(e[0]=n(`<h1 id="utility-subcommands"> 유틸리티 하위 명령어 <a class="headerlink" href="#utility-subcommands" title="영구 링크"> ¶ </a></h1><p> 실거래(Live-Trade) 및 모의거래(Dry-Run) 모드 외에도, <code> 백테스팅 </code> , <code> 엣지 </code> 및 <code> 하이퍼옵트 </code> 최적화 하위 명령어와, 과거 데이터를 준비하는 <code> 데이터 다운로드 </code> 하위 명령어를 포함하여, 봇에는 여러 유틸리티 하위 명령어가 포함되어 있습니다. 이 섹션에서 설명합니다. </p><h2 id="create-userdir"> 사용자 디렉토리 생성 <a class="headerlink" href="#create-userdir" title="영구 링크"> ¶ </a></h2><p> Freqtrade 파일을 저장할 디렉토리 구조를 생성합니다. 시작할 수 있도록 전략 및 하이퍼옵트 예제도 생성됩니다. 여러 번 사용할 수 있으며, <code> --reset </code> 옵션을 사용하면 샘플 전략 및 하이퍼옵트 파일이 기본 상태로 재설정됩니다. </p><div class="highlight"><pre><code>사용법: freqtrade create-userdir [-h] [--userdir 경로] [--reset]
...
? 거래소를 선택하세요  binance
? 텔레그램을 활성화하시겠습니까?  아니오
</code></pre></div><h2 id="show-config"> 구성 표시 <a class="headerlink" href="#show-config" title="영구 링크"> ¶ </a></h2><p> 구성 파일을 표시합니다 (기본적으로 민감한 값은 숨김 처리됨). 특히 <a href="configuration.html"> 분할 구성 파일 </a> 또는 <a href="configuration.html"> 환경 변수 </a> 를 사용하는 경우, 이 명령은 병합된 구성을 표시합니다. </p>`,15)),s("p",null,[s("img",{alt:"구성 출력 표시",src:r(t)},null,8,d)]),e[1]||(e[1]=n(`<div class="highlight"><pre><code>사용법: freqtrade show-config [-h] [--userdir 경로] [-c 경로]
                             [--show-sensitive]
...
</code></pre></div><h2 id="list-strategies"> 전략 목록 <a class="headerlink" href="#list-strategies" title="영구 링크"> ¶ </a></h2><p> 특정 디렉토리의 모든 전략을 보려면 <code> list-strategies </code> 하위 명령어를 사용하십시오. </p><p> 이 하위 명령어는 환경에서 전략을 로드하는 데 문제가 있는지 찾는 데 유용합니다: 오류가 있는 전략이 포함된 모듈은 빨간색으로 표시되고 (로드 실패), 중복 이름이 있는 전략은 노란색으로 표시됩니다 (중복 이름). </p><div class="highlight"><pre><code>사용법: freqtrade list-strategies [-h] [-v] [--no-color] [--logfile FILE] [-V]
