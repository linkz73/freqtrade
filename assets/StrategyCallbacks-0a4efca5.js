import{_ as a}from"./_plugin-vue_export-helper-c27b6911.js";import{f as n,o as p,c,a as l}from"./index-1d9d7dc5.js";const e=n({name:"StrategyCallbacks"}),t={class:"md-content__inner md-typeset"};function o(r,s,i,d,u,m){return p(),c("article",t,s[0]||(s[0]=[l(`<h1 id="strategy-callbacks"> 전략 콜백 <a class="headerlink" href="#strategy-callbacks" title="영구 링크"> ¶ </a></h1><p> 주요 전략 함수( <code> populate_indicators() </code>, <code> populate_entry_trend() </code>, <code> populate_exit_trend() </code> )는 벡터화된 방식으로 사용되어야 하며, <a href="bot-basics.html"> 백테스팅 중 한 번만 호출됩니다 </a>. 반면, 콜백은 &quot;필요할 때마다&quot; 호출됩니다. </p><p>따라서 콜백에서 무거운 계산을 수행하는 것을 피해야 하며, 이는 운영 중 지연을 초래할 수 있습니다. 사용된 콜백에 따라 거래 진입/종료 시 또는 거래 기간 동안 호출될 수 있습니다.</p><p>현재 사용 가능한 콜백:</p><ul><li><a href="#bot-start"><code> bot_start() </code></a></li><li><a href="#bot-loop-start"><code> bot_loop_start() </code></a></li><li><a href="#stake-size-management"><code> custom_stake_amount() </code></a></li><li><a href="#custom-exit-signal"><code> custom_exit() </code></a></li><li><a href="#custom-stoploss"><code> custom_stoploss() </code></a></li><li><a href="#custom-order-price-rules"><code> custom_entry_price() </code> 및 <code> custom_exit_price() </code></a></li><li><a href="#custom-order-timeout-rules"><code> check_entry_timeout() </code> 및 <code> check_exit_timeout() </code></a></li><li><a href="#trade-entry-buy-order-confirmation"><code> confirm_trade_entry() </code></a></li><li><a href="#trade-exit-sell-order-confirmation"><code> confirm_trade_exit() </code></a></li><li><a href="#adjust-trade-position"><code> adjust_trade_position() </code></a></li><li><a href="#adjust-entry-price"><code> adjust_entry_price() </code></a></li><li><a href="#leverage-callback"><code> leverage() </code></a></li><li><a href="#order-filled-callback"><code> order_filled() </code></a></li></ul><div class="admonition tip"><p class="admonition-title">콜백 호출 순서</p><p> 콜백 호출 순서는 <a href="bot-basics.html"> bot-basics </a>에서 확인할 수 있습니다. </p></div><h2 id="imports-necessary-for-a-strategy"> 전략에 필요한 임포트 <a class="headerlink" href="#imports-necessary-for-a-strategy" title="영구 링크"> ¶ </a></h2><p>전략을 만들 때 필요한 모듈과 클래스를 임포트해야 합니다. 전략에 필요한 임포트는 다음과 같습니다:</p><p>기본적으로, 전략의 기본 라인으로 다음 임포트를 권장합니다: 이는 freqtrade 기능이 작동하는 데 필요한 모든 임포트를 포함합니다. 필요에 따라 더 많은 임포트를 추가할 수 있습니다.</p><div class="highlight"><pre><span></span><code><span class="c1"># flake8: noqa: F401</span>
...
</code></pre></div><h2 id="bot-start"> 봇 시작 <a class="headerlink" href="#bot-start" title="영구 링크"> ¶ </a></h2><p>전략이 로드될 때 한 번 호출되는 간단한 콜백입니다. 이는 데이터 제공자와 지갑이 설정된 후 한 번만 수행해야 하는 작업을 수행하는 데 사용할 수 있습니다.</p><div class="highlight"><pre><span></span><code><span class="kn">import</span><span class="w"> </span><span class="nn">requests</span>
...
</code></pre></div><h2 id="stake-size-management"> 스테이크 크기 관리 <a class="headerlink" href="#stake-size-management" title="영구 링크"> ¶ </a></h2><p>거래에 진입하기 전에 호출되며, 새로운 거래를 할 때 포지션 크기를 관리할 수 있습니다.</p><div class="highlight"><pre><span></span><code><span class="c1"># Default imports</span>
...
</code></pre></div><p> 전체 예제는 <a href="strategy-callbacks.html"> 사용자 정의 손절매 </a> 섹션에서 찾을 수 있습니다. </p></details><div class="admonition note"><p class="admonition-title">참고</p><p><code> stoploss_from_open() </code> 에 잘못된 입력을 제공하면 &quot;CustomStoploss 함수가 유효한 손절매를 반환하지 않았습니다&quot; 경고가 발생할 수 있습니다. 이는 <code> current_profit </code> 매개변수가 지정된 <code> open_relative_stop </code> 보다 낮은 경우 발생할 수 있습니다. 이러한 상황은 <code> confirm_trade_exit() </code> 메서드에 의해 거래 종료가 차단될 때 발생할 수 있습니다. 경고는 손절매 판매를 차단하지 않도록 <code> confirm_trade_exit() </code> 메서드에서 <code> exit_reason </code> 을 확인하거나 <code> return stoploss_from_open(...) or 1 </code> 관용구를 사용하여 해결할 수 있습니다. 이는 <code> current_profit &lt; open_relative_stop </code> 인 경우 손절매를 변경하지 않도록 요청합니다. </p></div><h4 id="stoploss-percentage-from-absolute-price"> 절대 가격에서 손절매 백분율 <a class="headerlink" href="#stoploss-percentage-from-absolute-price" title="영구 링크"> ¶ </a></h4><p><code> custom_stoploss() </code> 에서 반환된 손절매 값은 항상 <code> current_rate </code> 에 대한 백분율을 지정해야 합니다. 지정된 절대 가격 수준에서 손절매를 설정하려면 <code> stop_rate </code> 를 사용하여 현재 가격에 대한 백분율을 계산해야 합니다. 이는 백분율이 진입 가격에서 지정된 것과 동일한 결과를 제공합니다. </p><p> 도우미 함수 <code> stoploss_from_absolute() </code> 는 절대 가격에서 <code> custom_stoploss() </code> 에서 반환될 수 있는 현재 가격 상대 손절매로 변환하는 데 사용할 수 있습니다. </p><details class="example"><summary>사용자 정의 손절매 함수에서 절대 가격을 사용한 손절매 반환</summary><p> 현재 가격보다 2xATR 아래에 손절매를 설정하려면 <code> stoploss_from_absolute(current_rate + (side * candle[&quot;atr&quot;] * 2), current_rate=current_rate, is_short=trade.is_short, leverage=trade.leverage) </code> 를 호출할 수 있습니다. 선물의 경우 방향(상승 또는 하락)을 조정하고 레버리지를 조정해야 합니다. 이는 <a href="strategy-callbacks.html"><code> custom_stoploss </code></a> 콜백이 <a href="stoploss.html"> &quot;이 거래의 위험&quot; </a> 을 반환하기 때문입니다. 이는 상대 가격 이동이 아닙니다. </p><div class="highlight"><pre><span></span><code><span class="c1"># Default imports</span>
...
<span class="sd">        &quot;&quot;&quot;</span>
        ...
        <span class="k">return</span> <span class="kc">None</span>
</code></pre></div>`,164)]))}const h=a(e,[["render",o]]);export{h as default};
